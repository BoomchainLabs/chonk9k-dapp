...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString() ?: throw EarnAppInvalidDataException("Value is not a string")
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw EarnAppInvalidDataException("`$name` is not set")
            is JsonNull -> throw EarnAppInvalidDataException("`$name` is null")
            else -> throw EarnAppInvalidDataException("`$name` is invalid, received $this")
        }
